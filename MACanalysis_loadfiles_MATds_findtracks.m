%%%=== MACanalysis_loadfiles_MATds_findtracks ===%%%

% This is script 1/5.

% It loads raw spm files from a MATLAB data structure, so the Nanoscope
% MATLAB toolbox is not required to run the script.
...
DataFolder   = 'Z:\Users\George\Documents\PhD\MAC_Manuscript\MAC_Analysis_Everything\EPFL1_experiment\DataStructures';
% Save outputs to:
outdirectory = 'Z:\Users\George\Documents\PhD\MAC_Manuscript\MAC_Analysis_Everything\EPFL1_experiment\DataStructures';

DataStructure_LoadName = 'MAC_EPFL1_ImagesOnly_DataStructure_Files_0to478';
DataStructure_SaveName = 'MAC_EPFL1_tracktable';
...
LoadFullFileName = fullfile(DataFolder, DataStructure_LoadName);
load(strcat(LoadFullFileName, '.mat'));

Vid_Seq_Raw = MAC_tracking_data.Vid_Seq_Raw;

display('Creating file names and loading image sequence...')
% pre-allocate cell array for loaded spm files (nm)
Vid_Seq_Mat = cell(size(Vid_Seq_Raw));

for i = 1:length(Vid_Seq_Raw)
    
    heightdata = Vid_Seq_Raw{i};
...
%% Review accuracy of tracking with video
